 ...
 *
 * Deno mirror of `src/config/tradeGuardrails.ts`. Keep the decision logic in
 * sync with that file; the server-only database helpers at the bottom exist
 * here only.
 *
...
import { EU_COUNTRIES, type TaxTreatment } from "./taxRules.ts";
...
/** Load (and lazily seed) the net-terms credit file for an account. */
export async function loadCreditProfile(
  admin: MinimalClient,
  userId: string,
): Promise<TradeCreditProfile | null> {
  const { data } = await admin
    .from("trade_credit_profiles")
    .select(
      "approved_credit_limit_eur_cents, first_order_limit_eur_cents, net_terms_enabled, settled_order_count, outstanding_balance_eur_cents",
    )
    .eq("user_id", userId)
    .maybeSingle();
  if (data) return data as TradeCreditProfile;

  // Approved accounts created before the guardrail migration: seed the
  // conservative default rather than letting the order through unchecked.
  const { data: seeded } = await admin
    ...
    .maybeSingle();
  return (seeded as TradeCreditProfile) ?? null;
}

/** Convert an order total into the EUR cents the credit ceiling is denominated in. */
export async function toEurCents(
  admin: MinimalClient,
  cents: number,
  currency: string,
): Promise<number> {
  const from = String(currency || "eur").toUpperCase();
  if (from === "EUR") return Math.round(cents);
  const { data } = await admin
    .from("currency_rates")
    .select("rate")
  ...
  const { data: inverse } = await admin
    ...
    .maybeSingle();
  const inv = Number((inverse as { rate?: number } | null)?.rate);
  if (Number.isFinite(inv) && inv > 0) return Math.round(cents / inv);

  // No rate available: treat the amount as EUR so the ceiling still bites.
  return Math.round(cents);
}

/** Restricted-allocation metadata for the pieces in a cart. */
export async function loadAllocationRules(
  admin: MinimalClient,
  pickIds: string[],
): Promise<Map<string, { restricted: boolean; cap: number | null; stock: number | null }>> {
  const map = new Map<string, { restricted: boolean; cap: number | null; stock: number | null }>();
  ...
  for (const row of (data ?? []) as Record<string, unknown>[]) {
    const entry = {
      restricted: row.is_allocation_restricted === true,
      cap: Number.isFinite(Number(row.allocation_unit_cap)) ? Number(row.allocation_unit_cap) : null,
      stock: Number.isFinite(Number(row.available_stock_units)) ? Number(row.available_stock_units) : null,
    };
    if (row.source_pick_id) map.set(String(row.source_pick_id), entry);
    if (row.id) map.set(String(row.id), entry);
  }
  return map;
}
