...
SOURCE INTERPRETATION (read the source image first, then follow the matching rule):
- If the source is already a 3D axonometric / perspective interior render: preserve the EXISTING scene exactly — same walls, openings, floor edges, fixed elements, camera angle, lens, lighting, and every piece of furniture already present. Only ADD the new product images at sensible empty positions; do not redesign the room.
- If the source is a 2D floor plan, elevation, or section drawing (top-down line drawing, no perspective, with walls/doors/windows shown as 2D symbols): treat the plan as a STRICT LAYOUT BLUEPRINT. Lift it into a 3D axonometric view (≈60–75° elevation, dollhouse-style top-down) while preserving the EXACT footprint, wall positions, door/window openings, and room proportions. Then place each provided product at the SAME XY position it (or an equivalent placeholder block) occupies in the plan, with the same orientation/rotation indicated by the 2D symbol. Do NOT invent additional furniture, rearrange the layout, or change room dimensions. If the plan shows a sofa against the north wall, the rendered sofa must be against that same wall in the output.

PRODUCT FIDELITY: Reproduce each provided product's exact silhouette, materials, and proportions from its photo. Match scale to the room (use door height ≈2.1m as reference). Add physically plausible shadows and contact points so each product sits naturally on the floor / against the wall.

CRITICAL: Do not crop, rotate, or re-frame the scene relative to the source. The architectural shell in your output must align with the source. Style: ${defaultStyle}.`;
