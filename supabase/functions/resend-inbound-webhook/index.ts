...
function renderKeyEmail(lead: Lead, portalUrl: string): string {
  const paragraphs = [
...
async function fetchInboundEmail(emailId: string): Promise<{
  text: string;
...
serve(async (req) => {
  ...
  for (const address of toList) {
    ...
    if (UUID_RE.test(tag)) {
      const { data: byTag } = await supabase
        ...
        .select("id, studio_name, founder_name, business_email, executive_emails, campaign_status, portal_key_sent_at")
        .eq("id", tag)
        .maybeSingle();
  ...
  if (!lead && fromEmail) {
    const { data: candidates } = await supabase
      .from("acquisition_leads")
      .select("id, studio_name, founder_name, business_email, executive_emails, campaign_status, portal_key_sent_at, outbound_recipients")
      .or(
  ...
  const fetched = emailId ? await fetchInboundEmail(emailId) : null;
  if (!fetched) {
    return finish(
