...
begin
  ...
  scored as (
    select
      u.*,
      (
        (case when f_category    is null or u.category    ilike '%'||f_category||'%'    then 1 else 0 end)
      + (case when f_subcategory is null or u.subcategory ilike '%'||f_subcategory||'%' then 1 else 0 end)
      + (case when f_designer    is null or u.designer    ilike '%'||f_designer||'%'    then 1 else 0 end)
      + (case
           when f_max_lead_wks is null then 1
      ...
      /
      nullif(
        (case when f_category    is null then 0 else 1 end)
  ...
  select
    ...
    s.similarity,
    coalesce(s.structural_fit_raw, 1.0) as structural_fit
  from scored s
  where
  ...
  order by (0.7 * s.similarity + 0.3 * coalesce(s.structural_fit_raw,1.0)) desc
  limit match_count;
end;
$$;
