...
async function expectStableMobileCommerceDock(page: Page, state: string) {
  const geometry = await page.locator("[data-mobile-commerce-dock]").evaluate((element) => {
    ...
    return {
      ...
      shellBottom: Math.round(
        document.querySelector(".product-page-root")?.getBoundingClientRect().bottom ?? 0,
      ),
      paddingBottom: Number.parseFloat(style.paddingBottom),
    };
  });

  expect(geometry.position, `${state}: dock positioning`).toBe("sticky");
  ...
  expect(geometry.rectBottom, `${state}: dock must end at the shell edge`).toBe(
    geometry.shellBottom,
  );
...
test.describe("Public product iOS canvas", () => {
  ...
  test("keeps the commerce dock sticky through scroll and dynamic viewport changes", async ({
    page,
  }, testInfo) => {
    ...
    await expect(page.locator("[data-mobile-commerce-buffer]")).toHaveCSS("height", "96px");
  });
