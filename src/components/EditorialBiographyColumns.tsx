  ...
  if (block.kind === "video") {
    return (
      <figure className="h-auto m-0">
        ...
        <div className="max-h-[360px] overflow-hidden [&_*]:rounded-none">
          <VideoBlock
            url={block.url}
  ...
  return (
    <figure className="h-auto m-0">
      ...
      <img
        ...
        className="w-full h-auto max-h-[360px] object-cover bg-muted/20 rounded-none"
        loading="lazy"
        decoding="async"
      />
    </figure>
...
export default function EditorialBiographyColumns({
  ...
  return (
    <div className="bg-cream">
      <div className="mx-auto max-w-5xl px-6 md:px-12 pt-6 md:pt-10 pb-10 md:pb-14">
        <div className="flex flex-col">
          {rows.map((row, i) => (
            <div key={`row-${i}`} className="h-auto py-4 md:py-5 first:pt-0 last:pb-0">
              {/* Ultra-fine horizontal baseline rule — locks left→right reading flow */}
              <div className="w-full h-px bg-foreground/10 mb-4 md:mb-5" />
        ...
        {footer && (
          <div className="pt-8 md:pt-10 transition-all duration-700 ease-out opacity-100 translate-y-0">
            {footer}
          </div>
        )}
      </div>
