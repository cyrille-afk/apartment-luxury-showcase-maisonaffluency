...
async function resolveCommitFinishes(pickIds: string[]): Promise<CommitFinishSnapshot[]> {
  ...
  const byPickAndId = new Map<string, { name: string; category: string | null }>();
  (data || []).forEach((row: any) => {
    byPickAndId.set(`${row.pick_id}::${row.fabric_id}`, { name: row.name, category: row.category ?? null });
  });

  return persisted.map(({ pickId, saved }) => {
    const rowFor = (id?: string | null) => id ? byPickAndId.get(`${pickId}::${id}`) ?? null : null;
    const fabric = rowFor(saved?.fabricId)?.name ?? null;
    const base = rowFor(saved?.baseId)?.name ?? null;
    const topRow = rowFor(saved?.topId);
    const top = topRow?.name ?? null;
    const topIsFabric = isFabricFinishCategory(topRow?.category);
    return {
      pick_id: pickId,
      variant_label: null,
      fabric_label: joinFinishLabels(fabric, topIsFabric ? top : null),
      wood_label: joinFinishLabels(base, topIsFabric ? null : top),
    };
...
export function TearsheetProposalCard({ proposal, onResolved, excluded: excludedProp, onExcludedChange, locked: lockedProp, onLockedChange, newPickIds }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-accent/40 bg-accent/[0.04] p-3.5 my-2 animate-fade-in">
      ...
      {/* Workflow hint — concise concierge-style guidance. The lock-count
          badge carries most of the state weight so the copy stays minimal. */}
      {status === "pending" && (
        <div className="mb-3 rounded-lg border border-accent/30 bg-accent/[0.05] px-3 py-2.5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 mb-0.5">
                <Sparkles className="h-3 w-3 text-accent shrink-0" />
                <span className="font-display text-[10px] uppercase tracking-widest text-accent">
                  Configure before approving
                </span>
              </div>
              <p className="font-body text-[10.5px] text-foreground/70 leading-snug">
          ...
          {lockedVisible.length === 0 && visiblePicks.length > 0 && (
            <p className="mt-1.5 font-body text-[10px] text-accent/80">
              Nothing locked yet — finishes will default to the concierge's suggestion.
            </p>
          )}
        </div>
      )}
