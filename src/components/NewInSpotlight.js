...
import { ArrowRight, Maximize2, Instagram } from "lucide-react";
...
const NewInSpotlight = ({ designer, showEyebrow = true, variant = "default", picksOverride, brandLabelOverride, pickDesignerSlugOverride, relatedPicksOverride }) => {
    const navigate = useNavigate();
    const hasOverride = Array.isArray(picksOverride);
    const isParentBrand = !hasOverride && isParentBrandDesigner(designer);
    const { data: founderIsBrand = false } = useFounderIsBrand(isParentBrand ? undefined : designer.founder);
    ...
    const lightboxItems = useMemo(() => picks.map(toLightboxItem), [picks, designer.name, designer.slug, brandLabelOverride, pickDesignerSlugOverride]);
    // Sibling catalogue for the lightbox "More from" strip. Falls back to the
    // displayed picks when no override is supplied.
    const lightboxRelatedItems = useMemo(() => (relatedPicksOverride ? relatedPicksOverride.map(toLightboxItem) : lightboxItems), [relatedPicksOverride, lightboxItems]);
    ...
    const slugByName = useMemo(() => {
        ...
        for (const d of allDesigners) {
            if (d.name)
                map.set(key(d.name), d.slug);
            if (d.display_name)
                map.set(key(d.display_name), d.slug);
            if (d.name?.includes(" - "))
                map.set(key(d.name.split(" - ")[0]), d.slug);
        }
        return map;
    }, [allDesigners]);
    const resolveSlug = (label) => {
        if (!label)
            return null;
        ...
        if (currentDesignerNames.includes(normalizedLabel))
    ...
    const firstBioParagraph = useMemo(() => {
        ...
        for (const block of blocks) {
            ...
            if (/^https?:\/\//i.test(firstToken) && !/\s/.test(firstToken))
                continue;
            text = block;
            break;
        }
    ...
    const renderCuratorsPicksSection = ({ barClassName = "flex items-center justify-between mb-6", titleClassName = "hidden md:block font-sans text-[11px] font-medium uppercase tracking-[0.2em] text-foreground", mobileBadgeClassName = "px-4 py-1.5 rounded-full border border-foreground/20 bg-foreground/5 md:hidden", mobileTitleClassName = "font-display text-[11px] md:text-xs tracking-[0.2em] uppercase text-foreground font-semibold", fullWidthDesktop = false, } = {}) => (<>
      <div className={barClassName}>
        ...
        <div className="flex items-center gap-2">
          {/* Mobile toggle */}
          <button onClick={() => setMobileGridCols((prev) => (prev === 1 ? 2 : 1))} className="md:hidden flex items-center p-1.5 rounded transition-all hover:opacity-70" aria-label={`Switch to ${mobileGridCols === 1 ? 2 : 1} column grid`}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              {mobileGridCols === 2 ? (<rect x="4" y="3" width="16" height="18" rx="1" fill="currentColor"/>) : (<>
          ...
          {/* Desktop toggle */}
          {!fullWidthDesktop && <button onClick={() => setGridCols((prev) => (prev === 3 ? 4 : 3))} className="hidden md:flex items-center p-1.5 rounded transition-all hover:opacity-70" aria-label={`Switch to ${gridCols === 3 ? 4 : 3} column grid`} title={gridCols === 3 ? "Display 4" : "Display 3"}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              {gridCols === 4 ? (<>
      ...
      <div className={cn("grid items-stretch gap-x-4 gap-y-8 md:gap-x-8 md:gap-y-10", mobileGridCols === 1 ? "grid-cols-1" : "grid-cols-2", fullWidthDesktop ? "md:grid-cols-3" : gridCols === 4 ? "md:grid-cols-4" : "md:grid-cols-3")}>
        {picks.map((pick) => {
            const alternateImage = pick.hover_image_url
                ...
                || null;
            return (<div key={pick.id} className="group flex h-full min-w-0 flex-col cursor-pointer" onClick={() => {
              ...
              <div className="relative aspect-[320/393] w-full flex-none overflow-hidden rounded-sm bg-[hsl(var(--product-canvas))]">
                ...
                {(pick.pdf_url || (pick.pdf_urls && pick.pdf_urls.length > 0)) && (<div className="absolute bottom-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <SpecSheetButton pdfUrl={pick.pdf_url} pdfUrls={pick.pdf_urls} brandName={designer.name} productName={pick.title} variant="icon"/>
                  </div>)}
              </div>
              ...
              <div className="mt-2.5 grid w-full grid-rows-[1.25rem_2.75rem_1.25rem] items-start text-left leading-relaxed">
                {(() => {
                    ...
                    const attribution = isParentBrand
                        ...
                        : { title: composed.title, designer: undefined };
                    ...
                    const attributedDesigner = isParentBrand
                        ? (attribution.designer || pick.designer_name || "").trim()
                        : "";
                    ...
                    const brandLine = (brandLabelOverride
                        || attributedDesigner
                        || parentBrand
                    ...
                    return (<>
                      {/* Designer / brand — top, prominent */}
                       {brandSlug ? (<Link to={`/designers/${brandSlug}`} onClick={(e) => e.stopPropagation()} className="block h-5 font-display text-[11px] md:text-sm font-medium uppercase tracking-widest md:tracking-[0.18em] text-muted-foreground md:text-foreground leading-tight line-clamp-1 hover:underline underline-offset-4 decoration-foreground/40 transition-colors">
                            {brandLine}
                          </Link>) : brandLine ? (<span className="block h-5 font-display text-[11px] md:text-sm font-medium uppercase tracking-widest md:tracking-[0.18em] text-muted-foreground md:text-foreground leading-tight line-clamp-1">
                            {brandLine}
                          </span>) : (<span aria-hidden="true" className="block h-5"/>)}
    ...
    return (<>
      ...
      {isUnderlaid && (<section className="hidden md:block w-full bg-transparent">
          <div className="grid grid-cols-2 gap-x-10 items-start w-full">
            ...
            <div className="flex flex-col">
              <div className="aspect-[16/10] max-h-[450px] w-full overflow-hidden bg-[hsl(var(--canvas))]">
                <CldPicture src={portraitImage} alt={`${displayName} portrait`} className="w-full h-full object-cover" style={{ objectPosition: designer.hero_image_position || "center" }}/>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-start pt-0 pb-3">
              <div className="flex items-center gap-3 w-full">
                ...
                <ShareMenu url={shareUrl} message={`Maison Affluency · ${displayName}: ${shareUrl}`} className="flex items-center p-1 -m-1 text-foreground/40 hover:text-foreground transition-colors" iconSize="w-4 h-4 md:w-5 md:h-5" showLabel={false}/>
              </div>
              ...
              <div className="mt-5 w-full">
                <PortraitCtaLink label="View The Full Portrait" className="text-[10px] uppercase tracking-widest text-neutral-800 font-medium inline-flex items-center gap-4" onClick={() => {
                if (ctaPressed)
                    return;
                setCtaPressed(true);
              ...
              {igWithImages.length > 0 && (<div className="mt-6 pt-4 border-t border-neutral-100 w-full flex flex-col">
                  <div className="flex gap-2.5 items-center h-16 md:h-20 overflow-hidden flex-shrink-0">
                    {igWithImages.slice(0, 6).map((post) => (<a key={post.id} href={post.post_url} target="_blank" rel="noopener noreferrer" className="group relative block h-full aspect-square flex-shrink-0 overflow-hidden bg-[hsl(var(--canvas))]">
                        <CldPicture src={post.image_url} alt={post.caption || `${displayName} — From the Studio`} className="h-full w-full object-contain object-center transition-transform duration-700 ease-out group-hover:scale-105"/>
      ...
      <section className={cn("max-w-7xl mx-auto px-6 md:px-12 lg:px-20 pt-10 md:pt-16 pb-4 md:pb-6", isUnderlaid && "md:hidden")}>
        <div className="flex flex-col md:flex-row gap-8 md:gap-14 items-start">
          {/* Portrait */}
          <motion.div key={`portrait-${designer.slug}`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }} className="w-full md:w-[38%] flex-shrink-0">
          ...
          {/* Name + Bio + CTA */}
          <motion.div key={`bio-${designer.slug}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ ...transition, delay: 0.2 }} className="flex-1 flex flex-col justify-start">
            {showEyebrow && (<span className="font-body text-[10px] uppercase tracking-[0.35em] text-muted-foreground mb-2 block">
                New In
              </span>)}
            <div className="flex items-center gap-3 mb-8">
              ...
              <ShareMenu url={shareUrl} message={`Maison Affluency · New In · ${displayName}: ${shareUrl}`} className="flex items-center p-1 -m-1 text-foreground/40 hover:text-foreground transition-colors" iconSize="w-4 h-4 md:w-5 md:h-5" showLabel={false}/>
            </div>
            ...
            window.setTimeout(() => navigate(`/designers/${designer.slug}/biography?from=new-in`), 380);
                ...
                <span className={cn("relative inline-flex items-center whitespace-nowrap pl-0 pr-14 transition-[padding] duration-300", "group-hover:pl-20 group-hover:pr-0", ctaPressed && "pl-20 pr-0")}>
                  <span className={cn("pointer-events-none absolute left-0 top-1/2 h-px w-12 -translate-y-1/2 bg-current opacity-0 transition-all duration-300", "translate-x-2 group-hover:translate-x-0 group-hover:opacity-100", ctaPressed && "translate-x-0 opacity-100")}/>
                  <span className="relative z-10">View The Full Portrait</span>
                  <span className={cn("pointer-events-none absolute right-5 top-1/2 h-px w-8 -translate-y-1/2 bg-current opacity-100 transition-all duration-300", "translate-x-0 group-hover:translate-x-6 group-hover:opacity-0", ctaPressed && "translate-x-6 opacity-0")}/>
                  <ArrowRight className={cn("pointer-events-none absolute right-0 top-1/2 h-3.5 w-3.5 -translate-y-1/2 transition-all duration-300 group-hover:-translate-x-1 group-hover:opacity-0", ctaPressed && "-translate-x-1 opacity-0")}/>
                </span>
              </button>
            </div>

            {/* From the Studio */}
            {igWithImages.length > 0 && (<div className="mt-10 pt-8 border-t border-border/30">
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-1">
                  {igWithImages.slice(0, 6).map((post, index) => (<a key={post.id} href={post.post_url} target="_blank" rel="noopener noreferrer" className={`group relative block aspect-square overflow-hidden bg-muted ${index >= 3 ? "hidden md:block" : ""}`}>
                      <CldPicture src={post.image_url} alt={post.caption || `${displayName} — From the Studio`} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-105"/>
                      <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/20 transition-colors duration-300 flex items-center justify-center">
                        <Instagram className="h-4 w-4 text-background opacity-0 group-hover:opacity-100 transition-opacity duration-300"/>
                      </div>
                    </a>))}
      ...
      {/* Separator */}
      <div className={cn("max-w-7xl mx-auto px-6 md:px-12", isUnderlaid && "md:hidden")}>
        <div className="border-t border-border/40"/>
      </div>

      {/* Curators' Picks */}
      <section className={cn("max-w-7xl mx-auto px-6 md:px-12 pt-4 md:pt-6 pb-6 md:pb-24", isUnderlaid && "md:hidden")}>
        {renderCuratorsPicksSection()}
      </section>

      <PublicProductLightbox product={lightboxItem} allPicks={lightboxRelatedItems} onClose={() => setLightboxItem(null)} onSelectRelated={(item) => setLightboxItem(item)}/>
    </>);
};
