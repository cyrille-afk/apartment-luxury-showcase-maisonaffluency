...
export default function AuthGateDialog({ open, onClose, action = "download this document" }: AuthGateDialogProps) {
  ...
  const content = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={onClose}>
      <div className="bg-background border border-border rounded-lg p-6 w-full max-w-sm data-[mode=signup]:max-w-lg shadow-xl relative" data-mode={mode} onClick={(e) => e.stopPropagation()}>
