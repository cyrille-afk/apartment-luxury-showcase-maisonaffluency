...
const CrossfadeImage: React.FC<{ src: string; alt: string; pointerEventsNone?: boolean; backdropClass?: string }> = ({
  ...
  const base = cn(
    "h-full w-full object-contain object-center p-6 rounded-luxury-sharp",
...
const ProductImageGallery: React.FC<ProductImageGalleryProps> = ({ images, alt, overlay, bottomRightOverlay, firstImageBadge, activeIndex: controlledIndex, activeIndexNonce, onIndexChange, caption, compact, mobileMenuItems, pickId }) => {
  ...
  const closePresentationAt = useCallback(() => {
    // Closing the editorial presentation always returns the gallery to
    // picture 1, regardless of where the user stopped inside the viewer.
    ...
    // Re-measure first, then animate the inline track back to the first image.
  ...
  return (
    <div className="flex gap-4 items-stretch">
      ...
      <div className="flex-1 flex w-full flex-col items-center gap-3 min-w-0">
        <div className="relative group w-full min-w-0" ref={inlineSwipeRef}>
        <div
          ref={frameRef}
          className={cn("product-image-frame relative aspect-square w-full overflow-hidden rounded-luxury-sharp bg-[hsl(var(--canvas))] transition-[width] duration-300 ease-out", compact && "product-image-frame--compact")}
        ...
        >
                ...
                {images.map((img, i) => (
                  <div key={`${img}-${i}`} className="flex h-full min-w-0 flex-[0_0_100%] items-center justify-center">
                    <img
                      ...
                      decoding="async"
                      className="h-full w-full select-none object-contain object-center p-6 rounded-luxury-sharp"
                    />
                  </div>
                ))}
              </div>
          ...
          {firstImageBadge && activeIndex === 0 && (
            <div
              className={cn(
                "absolute left-3 z-20 pointer-events-none",
                bottomRightOverlay ? "top-[calc(var(--gallery-control-offset,1rem)+3.25rem)]" : "top-3"
              )}
            >
