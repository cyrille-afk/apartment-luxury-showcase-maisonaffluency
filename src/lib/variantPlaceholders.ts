...
function placeholderFromAxisLabel(label: string): string {
  ...
  const titled = pretty
    ...
    .join(" ");
  const skipSuffix = /\b(base|top|upholstery|finish|fabric|material|size|colour|color|leather|diffuser|shade|shape|form|profile|model|version|variant|option)$/i.test(titled);
