...
/** Upload source image or PDF (with page selector) */
const SourceUpload = ({ onSourceReady }: { onSourceReady: (url: string) => void }) => {
  ...
  const loadPdfPreviews = async (file: File) => {
    ...
    try {
      ...
      // Generate small thumbnails for up to 20 pages
  ...
  const confirmPageSelection = async () => {
    ...
    try {
      ...
      const path = `axonometric-sources/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.png`;
